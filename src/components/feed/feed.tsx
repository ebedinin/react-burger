...
import { TExtFeed, TFeeds } from '../../services/api/type/order';
...
const Feed: FC<TFeedProps> = ({ feed }) => {
    ...
    const ingredients = feed.ingredients.slice(0, greaterMaximum).map((ingredient, index) =>
        <div className={`${style.containerIngredient}`}>
            <img className={`${style.ingredientImage}`} src={ingredient.image} alt='ingredient' />
...
const OrderAll: FC = () => {
    ...
    const { data, isSuccess } = useGetOrderQuery(orderNumber)
...
const FeedsAll: FC = () => {
    const { data, isSuccess } = useGetOrdersAllQuery()
...
const FeedsUser: FC = () => {
    const { data, isSuccess } = useGetOrdersUserQuery()
    if (!isSuccess || data === undefined || data.orders === undefined) return null
    return (
